...
describe('Booking Model', () => {
  it('should require all required attributes', () => {
    ...
    expect(error!.errors['total.local.total'].message).toBe('Path `total.local.total` is required.')
    expect(error!.errors['total.ttv.currencyId'].message).toBe(
      'Path `total.ttv.currencyId` is required.'
    )
    expect(error!.errors['total.ttv.total'].message).toBe('Path `total.ttv.total` is required.')
